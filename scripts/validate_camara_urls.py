...
def candidate_urls(name: str, uf: str) -> list[str]:
    ...
    for slug in slugs:
        candidates += [
            f"https://{slug}.{uf_lower}.leg.br/",
            f"https://www.{slug}.{uf_lower}.leg.br/",
            f"https://camara.{slug}.{uf_lower}.gov.br/",
            f"https://www.camara.{slug}.{uf_lower}.gov.br/",
            f"https://camara{slug}.{uf_lower}.gov.br/",
        ]
...
async def main():
    ...
    candidates = [
        ...
        if not row["camara_url"].strip()
    ]

    print(f"Checking {len(candidates)} municipalities for câmara URLs...")
    ...
    print(f"\nConfirmed: {confirmed}/{len(candidates)} câmara URLs found")
